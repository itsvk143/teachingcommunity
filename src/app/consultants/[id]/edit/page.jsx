...
import {
    CheckCircle, MapPin, AlertCircle, Building, User, Phone, Mail, Globe, ArrowLeft
} from 'lucide-react';
...
export default function EditConsultantPage({ params }) {
    ...
    useEffect(() => {
        const fetchConsultant = async () => {
            try {
                ...
                if (session?.user?.id && data.owner_user_id !== session.user.id && session.user.role !== 'admin') {
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="max-w-4xl mx-auto">
                <Link href="/dashboard" className="inline-flex items-center text-gray-500 hover:text-gray-700 mb-6 font-medium">
                    <ArrowLeft className="w-4 h-4 mr-2" /> Back to Dashboard
                </Link>
                <div className="mb-6">
                    <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Edit Profile</h1>
                    <p className="mt-2 text-gray-500">Update your consulting details and branding.</p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                    ...
                    {success && (
                        <div className="bg-green-50 border-l-4 border-green-500 p-4 m-6 mb-0 rounded-r text-green-700 text-sm flex items-center">
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-8">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    />
                                    <p className="mt-2 text-xs text-gray-500">Update your brand identity or logo.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-6 border-t border-gray-100 flex justify-end">
                            <button
                                type="submit"
                                disabled={saving}
                                className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center disabled:opacity-50"
                            >
                                {saving ? "Saving Changes..." : "Save Changes"} <CheckCircle className="ml-2 w-5 h-5" />
                            </button>
                        </div>
                    </form>
                </div>
