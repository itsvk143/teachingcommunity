...
import {
  ...
  Users, BookOpen, Trophy, School, Shield, Image, FileText
} from 'lucide-react';
...
// --- HELPER COMPONENT ---
const FormField = ({ label, name, type = "text", value, onChange, required = false, placeholder = "", options = null, rows = null, maxLength = null, className = "", icon: Icon, disabled = false, min = null }) => {
  const baseClasses = "w-full pl-10 pr-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-gray-700 placeholder-gray-400 disabled:bg-gray-50 disabled:text-gray-500";
...
export default function RegisterSchool() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') router.push('/login?callbackUrl=/schools/register');
  }, [status, router]);
  ...
  const [form, setForm] = useState({
    ...
    // We'll use simple text areas for comma-separated lists for arrays/galleries
  ...
  const nextStep = () => {
    if (step === 1 && (!form.name || !form.email || !form.phone_primary || !form.state || !form.city)) {
    ...
    <div className="min-h-screen bg-gray-50/50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-gray-500">Complete all sections to create a professional profile</p>
        </div>
          ...
          <div className="p-8">
            ...
            {step === 1 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                  ...
                  <FormField label="Email" name="email" type="email" value={form.email} onChange={handleChange} required icon={Mail} />
            ...
            {step === 2 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormField label="Principal Photo URL" name="principal_photo_url" value={form.principal_photo_url} onChange={handleChange} icon={Image} placeholder="https://..." />
                  ...
                  <FormField label="Office Timing" name="office_timing" value={form.office_timing} onChange={handleChange} placeholder="e.g. 9:00 AM - 4:00 PM" />
                </div>
            ...
            {step === 3 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField label="Class Range" name="class_range" value={form.class_range} onChange={handleChange} placeholder="e.g. Nursery-12" />
                    <FormField label="Academic Session" name="academic_session" value={form.academic_session} onChange={handleChange} placeholder="e.g. April-March" />
                    <FormField label="Student Count" name="student_count" type="number" value={form.student_count} onChange={handleChange} />
                    <FormField label="Teacher:Student Ratio" name="student_teacher_ratio" value={form.student_teacher_ratio} onChange={handleChange} placeholder="1:30" />
                  </div>
            ...
            {step === 4 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <FormField label="Campus Area" name="campus_area" value={form.campus_area} onChange={handleChange} placeholder="e.g. 2 Acres" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField label="Student Pickup Policy" name="student_pickup_policy" value={form.student_pickup_policy} onChange={handleChange} placeholder="Short description" />
                    <FormField label="Visitor Entry Policy" name="visitor_entry_policy" value={form.visitor_entry_policy} onChange={handleChange} placeholder="Short description" />
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 5 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormField label="School Logo URL" name="logo_url" value={form.logo_url} onChange={handleChange} icon={Image} placeholder="https://..." />
                  <FormField label="Cover Image URL" name="cover_image_url" value={form.cover_image_url} onChange={handleChange} icon={Image} placeholder="https://..." />

                  <div className="md:col-span-2">
                    <FormField label="Gallery Images (Comma separated URLs)" name="gallery_images_str" value={form.gallery_images_str} onChange={handleChange} rows={3} placeholder="https://img1.jpg, https://img2.jpg" />
                  </div>
                  <div className="md:col-span-2">
                    <FormField label="Video Links (Comma separated)" name="video_gallery_str" value={form.video_gallery_str} onChange={handleChange} rows={2} placeholder="YouTube links..." />
                  </div>
                </div>
            ...
            <div className="flex justify-between items-center mt-10 pt-6 border-t border-gray-100">
              ...
              {step < steps.length ? (
                ...
                <button type="submit" disabled={loading} className="flex items-center px-8 py-2.5 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 shadow-md disabled:opacity-70">
                  {loading ? 'Submitting...' : 'Register School'} {loading ? '' : <CheckCircle className="w-5 h-5 ml-2" />}
                </button>
              )}
            </div>
