...
export default function VacanciesClient({ initialVacancies }) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters, searchTerm, viewCategory]);
  ...
  // Initial vacancies are loaded via Server Component, so we don't fetch on mount.


  useEffect(() => {
    if (status === 'authenticated' && session?.user?.role === 'user' && !syncingRole && !needsRelogin) {
  ...
  const paginatedVacancies = filteredVacancies.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Pagination Controls */}
            {filteredVacancies.length > ITEMS_PER_PAGE && (
              <div className="flex justify-center items-center gap-4 mt-12 bg-white px-6 py-4 rounded-xl shadow-sm border border-gray-100 max-w-md mx-auto">
                <button
                  onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                  disabled={currentPage === 1}
                  className="px-4 py-2 font-medium bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 disabled:opacity-50 transition-colors"
                >
                  Previous
                </button>
