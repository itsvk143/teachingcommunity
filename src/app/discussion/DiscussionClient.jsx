...
import { MessageSquare, Plus, Clock, ThumbsUp, ThumbsDown } from "lucide-react";

export default function DiscussionClient({ initialDiscussions, allowedCategories, isUnregistered }) {
    ...
    const fetchDiscussions = async (category) => {
        ...
        try {
            const url = category === "All"
                ? "/api/discussions"
                : `/api/discussions?category=${encodeURIComponent(category)}`;
            ...
            <div className="container mx-auto px-4 max-w-5xl relative z-10">
                ...
                <div className="flex overflow-x-auto pb-6 mb-4 gap-3 hide-scrollbar">
                    {allowedCategories.map((cat) => (
                        <button
                            key={cat}
                            onClick={() => fetchDiscussions(cat)}
                            disabled={loading}
                            className={`whitespace-nowrap px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-200 shadow-sm disabled:opacity-70 ${activeCategory === cat
                                    ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-blue-200 border-transparent transition-none"
                                    : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-900 cursor-pointer"
                                }`}
                        >
                            {cat}
                        </button>
